...
public interface AccomMapper {
    ...
    @Select("SELECT a.user_id, a.accom_id,r.rating, r.comment " +
    ...
    @Select("SELECT a.user_id, a.accom_id, a.accom_name ,a.accom_location, a.accom_phone, a.accom_caution, a.accom_description ," +
            ...
            "WHERE a.accom_id = #{accom_id}")
    AccomDTO getOneByUser(int accom_id);
    ...
    AccomDTO getOneByAccom_id(int accom_id); //숙소 수정 시 불러올 데이터
    ...
    boolean insert(AccomVO vo); //숙소 등록

    @Delete("DELETE FROM ACCOMMODATION WHERE accom_id = #{accom_id}")
    boolean delete(int accom_id); // 숙소 삭제
    ...
    boolean update(AccomVO vo); // 숙소 수정
